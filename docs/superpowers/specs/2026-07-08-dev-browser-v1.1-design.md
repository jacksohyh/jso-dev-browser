...
- **Trust boundary & threat model:** origin is derived in the main process from the sender's top frame (`e.senderFrame`, `parent === null`), never from a renderer-supplied string — a page cannot spoof another origin or read/fill another site's credentials, and requests from subframes are refused. `autofill:secret` additionally verifies the requested `id` belongs to the caller's own origin before decrypting. Accepted residual risks (inherent to form autofill, matching Chrome): once the user explicitly selects and fills a credential, page JS in that same origin can read the filled field value; and the in-page dropdown is theoretically subject to click-redressing, though it can only ever offer the page's own-origin credentials into its own-origin form.
